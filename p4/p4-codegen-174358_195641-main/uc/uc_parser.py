...
class UCParser:
    ...
    def p_function_definition(self, p):
        ...
        """
        p[2].type.type = VarDecl(None, p[1])
    ...
    def p_declarator_1(self, p):
        ...
        """
        p[0] = Decl(ID(p[1], self._token_coord(p, 1)), None, None)
    ...
    def p_declarator_2(self, p):
        ...
        """
        if isinstance(p[1].type, ArrayDecl):
            p[1].type.type = ArrayDecl(None, p[3] if len(p) == 5 else None)
        else:
            p[1].type = ArrayDecl(None, p[3] if len(p) == 5 else None)
        p[0] = p[1]
    ...
    def p_declarator_3(self, p):
        ...
        """
        if len(p) == 5:
            p[1].type = FuncDecl(p[3], None)
        else:
            p[1].type = FuncDecl(None, None)
    ...
    def p_parameter_declaration(self, p):
        ...
        # print("AQUI", p[2])
        decl = p[2]
        while decl.type is not None:
            decl = decl.type
        decl.type = VarDecl(None, p[1])
        # p[2].type = VarDecl(None, p[1])
        p[0] = p[2]
    ...
    def p_declaration(self, p):
        ...
        for decl in p[2]:
            if isinstance(decl.type, FuncDecl):
                decl.type.type = VarDecl(None, p[1])
            elif isinstance(decl.type, ArrayDecl):
                while decl.type is not None:
                    decl = decl.type
                # if isinstance(decl.type.type, ArrayDecl):
                #     decl.type.type.type = VarDecl(None, p[1])
                # else:
                #     decl.type.type = VarDecl(None, p[1])
                decl.type = VarDecl(None, p[1])
            else:
    ...
    def p_iteration_statement_2(self, p):
        ...
        if len(p) == 10:
            ...
            # OBS: not sure if p[3] should be passed as DeclList
            p[0] = For(
    ...
    def p_print_statement(self, p):
        ...
        """
        p[0] = Print(p[3] if len(p) == 6 else None, self._token_coord(p, 1))
